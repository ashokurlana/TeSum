...
def validate(args, device_id, pt, step):
    ...
    tokenizer = AutoTokenizer.from_pretrained("ltrctelugu/bert_ltrc_telugu")
    print("Validate ltrc_tokenizer : ",tokenizer)

    #symbols = {'BOS': tokenizer.vocab['[unused0]'], 'EOS': tokenizer.vocab['[unused1]'], 'PAD': tokenizer.vocab['[PAD]'], 'EOQ': tokenizer.vocab['[unused2]']}

    #symbols = {'PAD': tokenizer.vocab['[PAD]']}
    symbols = { 'PAD': tokenizer.vocab['[PAD]'], 'SEP':tokenizer.vocab['[SEP]'],'CLS':tokenizer.vocab['[CLS]']}
...
def test_abs(args, device_id, pt, step):
    ...
    test_iter = data_loader.Dataloader(args, load_dataset(args, 'test', shuffle=False), args.test_batch_size, device, shuffle=False, is_test=True)## is_test=False may also work;


    #tokenizer = BertTokenizer.from_pretrained('bert-base-uncased', do_lower_case=True, cache_dir=args.temp_dir)
    #tokenizer = AutoTokenizer.from_pretrained("ltrctelugu/bert_ltrc_telugu") ## default is_fast=True
    #tokenizer = AutoTokenizer.from_pretrained("ltrctelugu/bert_ltrc_telugu", is_fast=False)

    tokenizer = AutoTokenizer.from_pretrained("ltrctelugu/bert_ltrc_telugu", use_fast=False)
    print("\ntest_abs ltrc_tokenizer : ",tokenizer)
...
def test_text_abs(args, device_id, pt, step):
    ...
    #tokenizer = BertTokenizer.from_pretrained('bert-base-uncased', do_lower_case=True, cache_dir=args.temp_dir)
    tokenizer = AutoTokenizer.from_pretrained("ltrctelugu/bert_ltrc_telugu")
    print("test_text_abs ltrc_tokenizer : ",tokenizer)
...
def train_abs_single(args, device_id):
    ...
    #tokenizer = BertTokenizer.from_pretrained('bert-base-uncased', do_lower_case=True, cache_dir=args.temp_dir)
    tokenizer = AutoTokenizer.from_pretrained("ltrctelugu/bert_ltrc_telugu")
    print("train_abs_single ltrc_tokenizer : ",tokenizer)

    #symbols = {'BOS': tokenizer.vocab['[unused0]'], 'EOS': tokenizer.vocab['[unused1]'],'PAD': tokenizer.vocab['[PAD]'], 'EOQ': tokenizer.vocab['[unused2]']}

    print("model.vocab_size : ",model.vocab_size)
    #symbols = {'PAD': tokenizer.vocab['[PAD]'],'EOS': tokenizer.vocab['[unused1]']}
    symbols = { 'PAD': tokenizer.vocab['[PAD]'], 'SEP':tokenizer.vocab['[SEP]'],'CLS':tokenizer.vocab['[CLS]']}
