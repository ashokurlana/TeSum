...
### Adding our telugu bert embeddings:
...
class BertData():
    def __init__(self, args):
        self.args = args
        print("Bertdata function:\n")
        print("args: ",args)
        print("\n")
        #self.tokenizer = BertTokenizer.from_pretrained('bert-base-uncased', do_lower_case=True)
        print("data_builder.py file -- calling bert_ltrc_telugu tokenizer is calling...")
        self.tokenizer = AutoTokenizer.from_pretrained("ltrctelugu/bert_ltrc_telugu")
    ...
    def preprocess(self, src, tgt, sent_labels, use_bert_basic_tokenizer=False, is_test=False):
        ...
        #tgt_subtokens_str = '[unused0] ' + ' [unused2] '.join([' '.join(self.tokenizer.tokenize(' '.join(tt), use_bert_basic_tokenizer=use_bert_basic_tokenizer)) for tt in tgt]) + ' [unused1]'


        #tgt_subtokens_str = ''.join([' '.join(self.tokenizer.tokenize(' '.join(tt), use_bert_basic_tokenizer=use_bert_basic_tokenizer)) for tt in tgt])



        tgt_subtokens_str = ''.join([' '.join(self.tokenizer.tokenize(' '.join(tt))) for tt in tgt])
...
def _format_to_bert(params):
    ...
    for d in jobs:
        ...
        #print("\n\nBertdata files formate - args : ",args)
        #print("\n")
...
def format_to_lines(args):
    ...
    for c, f in enumerate(glob.glob(pjoin(args.raw_path, '*.json'))):
        print("c , f: ",c,f)
    ...
    for corpus_type in ['train', 'valid', 'test']:
        ...
        print("corpus_type, a_lst : ",corpus_type, len(a_lst))
        #print("\n")
        pool = Pool(args.n_cpus)
        dataset = []
        p_ct = 0
