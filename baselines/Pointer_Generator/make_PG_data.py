...
'''
Separte all the text and summary files into separate input and output directories

'''
def create_in_out_files(i_files_path, o_files_path, data_path):
  pairs = read_jsonl(data_path)
  c=0
  for i in range(len(pairs)):
    sample_num = i
    in_file = os.path.join(i_files_path, str(sample_num)+".text.txt" )
    ou_file = os.path.join(o_files_path, str(sample_num)+".summary.txt")
    fname_i = open(in_file, "w", encoding="utf-8")
    fname_o = open(ou_file, "w", encoding="utf-8")
    
    article = pairs[i]['cleaned_text']
    summary = pairs[i]['summary']

    fname_i.write(str(article))
    fname_o.write(str(summary))
    c+=1
  # print("xl-sum Test set Size = %d "%c)
  ...
  with open(out_file, 'wb') as writer:
    for idx,s in enumerate(story_fname):
      ...
      if os.path.isfile(os.path.join(url_file, s)):
        ...
        story_out_file = os.path.join(url_output_file, s[:-8]+'summary.txt') ### Cross check THIS, if you get any error in imbalance of samples:
...
def main():
 ...
 ############################################################################# Tokenized Directories:
  ...
  if not os.path.exists(idev_tkd_dir): os.makedirs(idev_tkd_dir)
  ...
  if not os.path.exists(odev_tkd_dir): os.makedirs(odev_tkd_dir)

  #Run stanford tokenizer on both stories dirs, outputting to tokenized stories directories
  tokenize_stories(idev_dir, idev_tkd_dir)
  tokenize_stories(itrain_dir, itrain_tkd_dir)
  tokenize_stories(itest_dir, itest_tkd_dir)

  tokenize_stories(odev_dir, odev_tkd_dir)
  ...
  write_to_bin(idev_tkd_dir, odev_tkd_dir, os.path.join(finished_files_dir, "dev.bin"))
