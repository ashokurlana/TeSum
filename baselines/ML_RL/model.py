...
vocab_obj = data.Vocab(config.vocab_path, config.vocab_size)
pretrained_word_embedding = vocab_obj.load_pretrained_wordembeddings(config.emb_dim, vocab_obj)
...
class Model(nn.Module):
    def __init__(self):
        ...
        #self.embeds = nn.Embedding.from_pretrained(pretrained_word_embedding, freeze=True)
        #self.embeds.weight = nn.Parameter(pretrained_word_embedding, requires_grad=False) ##freeze=True,regard=False weights wont update

        self.embeds = nn.Embedding.from_pretrained(pretrained_word_embedding, freeze=False)
        self.embeds.weight = nn.Parameter(pretrained_word_embedding, requires_grad=True) ##freeze=True,regard=False weights wont update
