...
class Train(object):
    def __init__(self, opt):
        self.vocab = Vocab(config_tel.vocab_path, config_tel.vocab_size)
        self.batcher = Batcher(config_tel.train_data_path, self.vocab, mode='train',
                               batch_size=config_tel.batch_size, single_pass=False)
        self.opt = opt
    ...
    def save_model(self, iter):
        save_path = config_tel.save_model_path + "/%07d.tar" % iter
        T.save({
            "iter": iter + 1,
    ...
    def setup_train(self):
        ...
        self.trainer = T.optim.Adam(self.model.parameters(), lr=config_tel.lr)
        start_iter = 0
        if self.opt.load_model is not None:
            load_model_path = os.path.join(config_tel.save_model_path, self.opt.load_model)
    ...
    def train_batch_MLE(self, enc_out, enc_hidden, enc_padding_mask, ct_e, extra_zeros, enc_batch_extend_vocab, batch):
        ...
        for t in range(min(max_dec_len, config_tel.max_dec_steps)):
            ...
            log_probs = T.log(final_dist + config_tel.eps)
            ...
            is_oov = (x_t >= config_tel.vocab_size).long()                                              #Mask indicating whether sampled word is OOV
    ...
    def train_batch_RL(self, enc_out, enc_hidden, enc_padding_mask, ct_e, extra_zeros, enc_batch_extend_vocab, article_oovs, greedy):
        ...
        for t in range(config_tel.max_dec_steps):
            x_t = self.model.embeds(x_t)
        ...
        for i in range(len(enc_out)):
            ...
            oovs = article_oovs[i]
            #print("id_list = ",id_list,type(id_list),len(id_list))
...
if __name__ == "__main__":
    ...
    print("intra_encoder:", config_tel.intra_encoder, "intra_decoder:", config_tel.intra_decoder)
    print("Cuda status: ",T.cuda.is_available())
